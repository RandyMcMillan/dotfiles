...
let input\
while(true){input=await question('Enter your query: ')\
if(input==='q'){int.close()\
process.exit(0)}\
try{input=JSON.parse(input)}catch{console.log('Invalid query, try again (it has to follow JSON format)')\
continue}\
for await(const event of db.collection('events').find(input))console.log(event)}\
function question(prompt){let res\
