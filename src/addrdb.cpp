...
bool CBanDB::Read(banmap_t& banSet)
{
    if (fs::exists(m_banlist_dat)) {
        LogPrintf("banlist.dat ignored because it can only be read by " PACKAGE_NAME " version 22.x. Remove %s to silence this warning.\n", std::quoted(fs::PathToString(m_banlist_dat)));
    }
...
std::optional<bilingual_str> LoadAddrman(const std::vector<bool>& asmap, const ArgsManager& args, std::unique_ptr<CAddrMan>& addrman)
{
    ...
    try {
        ...
        addrman = std::make_unique<CAddrMan>(asmap, /* deterministic */ false, /* consistency_check_ratio */ check_addrman);
        LogPrintf("Creating peers.dat because the file was not found (%s)\n", std::quoted(fs::PathToString(path_addr)));
        DumpPeerAddresses(args, *addrman);
    } catch (const std::exception& e) {
        addrman = nullptr;
        return strprintf(_("Invalid or corrupt peers.dat (%s). If you believe this is a bug, please report it to %s. As a workaround, you can move the file (%s) out of the way (rename, move, or delete) to have a new one created on the next start."),
                         e.what(), PACKAGE_BUGREPORT, std::quoted(fs::PathToString(path_addr)));
    }
    return std::nullopt;
}
...
std::vector<CAddress> ReadAnchors(const fs::path& anchors_db_path)
{
    ...
    try {
        ...
        LogPrintf("Loaded %i addresses from %s\n", anchors.size(), std::quoted(fs::PathToString(anchors_db_path.filename())));
