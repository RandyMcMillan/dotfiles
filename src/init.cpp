...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (!args.IsArgSet("-cjdnsreachable")) {
        if (!args.GetArgs("-onlynet").empty() && IsReachable(NET_CJDNS)) {
            return InitError(
    ...
    const bool onlynet_used_with_onion{!args.GetArgs("-onlynet").empty() && IsReachable(NET_ONION)};
    ...
    if (!i2psam_arg.empty()) {
        ...
        if (!args.GetArgs("-onlynet").empty() && IsReachable(NET_I2P)) {
            return InitError(
