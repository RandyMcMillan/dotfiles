...
interface Node $Proxy.wrap("interfaces::Node") {
    ...
    getWarnings @7 (context :Proxy.Context, type :Text) -> (result :Text);
    getLogCategories @8 (context :Proxy.Context) -> (result :UInt32);
    baseInitialize @9 (context :Proxy.Context) -> (error :Text $Proxy.exception("std::exception"), result :Bool);
    ...
    isAddressTypeSet @32 (context :Proxy.Context) -> (result :Bool);
    ...
    loadWallet @47 (context :Proxy.Context, name :Text) -> (error :Text, warning: Text, result :Wallet.Wallet);
