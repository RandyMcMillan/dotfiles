...
interface Node $Proxy.wrap("interfaces::Node") {
    ...
    executeRpc @35 (context :Proxy.Context, command :Text, params :Text, uri :Text) -> (error :Text $Proxy.exception("std::exception"), rpcError :Text $Proxy.exception("UniValue"), result :Text);
    ...
    customWalletLoader @41 (context :Proxy.Context) -> (result :Wallet.WalletLoader) $Proxy.name("walletLoader");
...
struct NodeStats $Proxy.wrap("CNodeStats") {
    ...
    lastSend @3 :Int64 $Proxy.name("m_last_send");
    lastRecv @4 :Int64 $Proxy.name("m_last_recv");
    lastTXTime @5 :Int64 $Proxy.name("m_last_tx_time");
    lastBlockTime @6 :Int64 $Proxy.name("m_last_block_time");
    timeConnected @7 :Int64 $Proxy.name("m_connected");
