...
interface WalletLoader extends(Chain.ChainClient) $Proxy.wrap("interfaces::WalletLoader") {
    ...
    restoreWallet @3 (context :Proxy.Context, backupFile :Text, name :Text) -> (error :Common.BilingualStr, warning :List(Common.BilingualStr), result :Wallet);
    listWalletDir @4 (context :Proxy.Context) -> (result :List(Text));
    getWallets @5 (context :Proxy.Context) -> (result :List(Wallet));
