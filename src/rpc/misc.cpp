    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = Assert(EnsureNodeContext(request.context).init)->ipc()) {
                ...
                ipc->spawnProcess("bitcoin-node", [&](interfaces::Init& init) -> interfaces::Base& {
                    echo = init.makeEcho();
                    return *echo;
                });
            } else {
                // The current binary doesn't have IPC support, so just create
                // a local interfaces::Echo object and return it so the
                // `echoipc` RPC method will work, and the python test calling
                // `echoipc` doesn't have to care whether it is testing a
                // bitcoind process without IPC support, or a bitcoin-node
