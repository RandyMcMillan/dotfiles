...
//! rules should apply.
static bool TypedArg(uint32_t flags)
{
...
static inline std::optional<std::string> ConvertToString(const util::SettingsValue& value, bool typed_arg)
{
...
static inline std::optional<int64_t> ConvertToInt(const util::SettingsValue& value, bool typed_arg)
{
...
static inline std::optional<bool> ConvertToBool(const util::SettingsValue& value, bool typed_arg)
{
    if (value.isBool()) return value.get_bool();
    ...
    const char* context) const
{
    ...
    if (!TypedArg(*flags)) require &= ~(ALLOW_BOOL | ALLOW_INT | ALLOW_STRING);
