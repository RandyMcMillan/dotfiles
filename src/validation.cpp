...
static DownloadState GetDownloadState(bool init)
{
    if (!init) return DownloadState::POST_INIT;
    if (::fReindex) return DownloadState::INIT_REINDEX;
    return DownloadState::INIT_DOWNLOAD;
}
...
static bool NotifyHeaderTip() LOCKS_EXCLUDED(cs_main) {
    ...
    if (fNotify) {
        uiInterface.NotifyHeaderTip(GetDownloadState(fInitialBlockDownload), pindexHeader);
    }
    return fNotify;
}
    ...
    do {
...

        {
            ...
            if (pindexFork != pindexNewTip) {
                ...
                uiInterface.NotifyBlockTip(GetDownloadState(fInitialDownload), pindexNewTip);
            }
        }
...
bool CChainState::InvalidateBlock(BlockValidationState& state, const CChainParams& chainparams, CBlockIndex *pindex)
{
    ...
    if (pindex_was_in_chain) {
        uiInterface.NotifyBlockTip(GetDownloadState(IsInitialBlockDownload()), to_mark_failed->pprev);
    }
    return true;
}
