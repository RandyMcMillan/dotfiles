...
bool BerkeleyEnvironment::Open(bilingual_str& err)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        ...
        err = strprintf(_("Error initializing wallet database environment %s!"), std::quoted(fs::PathToString(Directory())));
        return false;
    }
...
bool BerkeleyDatabase::Verify(bilingual_str& errorStr)
{
    ...
    if (fs::exists(file_path))
    {
        ...
        if (result != 0) {
            errorStr = strprintf(_("%s corrupt. Try using the wallet tool bitcoin-wallet to salvage or restoring a backup."), std::quoted(fs::PathToString(file_path)));
            return false;
        }
    }
