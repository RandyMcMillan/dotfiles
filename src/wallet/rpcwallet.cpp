                                ...
                                std::vector<bilingual_str>& warnings)
{
    if (load_on_startup.isTrue() && !AddWalletSetting(chain, wallet_name)) {
        warnings.emplace_back(Untranslated("Wallet list setting could not be written, so wallet may be not loaded on node restart."));
    } else if (load_on_startup.isFalse() && !RemoveWalletSetting(chain, wallet_name)) {
        warnings.emplace_back(Untranslated("Wallet list setting could not be written, so wallet may be reloaded on node restart."));
    }
}
