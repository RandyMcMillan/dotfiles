...
BOOST_FIXTURE_TEST_CASE(BasicOutputTypesTest, AvailableCoinsTestingSetup)
{
    ...
    // Bech32m
    dest.Set(wallet->GetNewDestination(OutputType::BECH32M, ""));
    BOOST_ASSERT(dest);
    ...
    // Bech32
    dest.Set(wallet->GetNewDestination(OutputType::BECH32, ""));
    BOOST_ASSERT(dest);
    ...
    // P2SH-SEGWIT
    dest.Set(wallet->GetNewDestination(OutputType::P2SH_SEGWIT, ""));
    BOOST_ASSERT(dest);
    ...
    // Legacy (P2PKH)
    dest.Set(wallet->GetNewDestination(OutputType::LEGACY, ""));
    BOOST_ASSERT(dest);
