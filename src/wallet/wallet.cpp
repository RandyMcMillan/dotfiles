        ...
        if (fExisted || IsMine(tx) || IsFromMe(tx))
        {
            ...
            for (const CTxOut& txout: tx.vout) {
                ...
                for (const CKeyID &keyid : vAffected) {
                    std::map<CKeyID, ReserveKey>::const_iterator mi = keyPool.find(keyid);
                    if (mi != keyPool.end()) {
...
void CWallet::MarkReserveKeysAsUsed(const ReserveKey reserve_key)
{
    AssertLockHeld(cs_wallet);
    CWalletDB walletdb(*dbw);
    std::set<int64_t> *setKeyPool = reserve_key.internal ? &setInternalKeyPool : &setExternalKeyPool;
    auto it = setKeyPool->begin();
    while (it != std::end(*setKeyPool)) {
        const int64_t& index = *(it);
        if (index > reserve_key.index) break; // set*KeyPool is ordered
